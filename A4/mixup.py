...
def preProcessing():
    ...
    valid = os.path.join(baseDir ,'Datasets','valid_img.csv')
    test = os.path.join(baseDir ,'Datasets','test_img.csv')
    train_img=pd.read_csv(train)
    valid_img=pd.read_csv(valid)
    ...
    valid_img['class_num'] = valid_img['label'].map(real_labels)
...
def valid(valid_img):
    valid = img_gen.flow_from_dataframe(valid_img, directory = img,x_col = 'filename', y_col = 'label', 
                                  target_size =(220, 220), class_mode = 'categorical', shuffle = False,
                                  batch_size = 12, color_mode = 'rgb')
    loss,acc = final_model.evaluate_generator(valid,steps=valid.n//12)
    return acc  
